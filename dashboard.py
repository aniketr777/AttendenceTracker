...
def login_student(roll_no, dob):
    try:
        response = requests.post(f"{API_BASE_URL}/api/login", 
                               json={"roll_no": roll_no, "dob": dob}, timeout=5)
        ...
        return {
            ...
            "attendance_summary": {
                ...
                "subjects_at_risk": sum(1 for s in student['attendance'].values() if s['percentage'] < 75)
            },
            "subjects": student['attendance']
        }

def get_color(p):
    if p >= 75: return "#3fb950"
    elif p >= 65: return "#d29922"
    return "#f85149"
...
if not st.session_state.authenticated:
    ...
    roll = st.text_input("Roll Number", placeholder="Enter your roll no").upper().strip()
    dob = st.text_input("Date of Birth (DD-MM-YYYY)", type="password", placeholder="DD-MM-YYYY")
    ...
    with col3:
        st.metric("At Risk", summary['subjects_at_risk'], help="Subjects below 75%")
